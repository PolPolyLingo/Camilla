 ...
 *-----------------------------------------------------------------------
 *
 * The code that did not exist in the original log4vala is shown below.
 * - Process to create if the log file save directory does not exist.
 *   (refresh_filehandle())
 *
...
namespace Log4Vala.Appender {
    ...
    public class FileAppender : Object, IAppender {
        ...
        private void refresh_filehandle () {
            var dirname = Camilla.Core.File.dirname (filename);
            if (!Camilla.Core.File.isDir (dirname)) {
                if (!Camilla.Core.File.mkdirs (dirname)) {
                    stderr.printf ("Unable to make directory for log file '%s'", filename);
                    return;
                }
            }
            try {
